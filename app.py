...
class UniversalCubicPlotterApp:
    ...
    def update_cartesian_plot(self):
        ...
        for collection in self.ax1.collections: collection.remove()
    ...
    def draw_polar_general(self):
        ...
        # <<< ИСПРАВЛЕНИЕ: Всегда сбрасываем вид к стандартному радиусу при новой отрисовке >>>
        # Это обеспечивает предсказуемое поведение кнопки "Построить / Обновить".
    ...
    def set_parabola_example(self):
        ...
        self.draw_plots_general()
        # Для параболы подходит общий масштаб по умолчанию, но мы все равно его явно задаем для надежности
        self.ax2_polar.set_ylim(0, self.default_rlim)
        self.canvas.draw_idle()
